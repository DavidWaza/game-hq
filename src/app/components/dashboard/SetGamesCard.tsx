...
const StatusCard: React.FC<StatusCardProps> = ({
  ...
  return (
    ...
    >
      <td
        className={`${getTdClasses(true, false)} md:rounded-tl-2xl md:rounded-bl-2xl`}
      >
        <div className="flex gap-3 items-center text-white w-full">
          ...
          <div className="flex-grow text-left">
            ...
            {status && (
              <p className="text-[#FCF8DB] text-xs font-semibold">
                ● {status.toUpperCase()}
              </p>
            )}
      ...
      <td className={getTdClasses()}>
        <div className="flex justify-between items-center md:block md:text-left">
          <h4 className="text-gray-400 text-sm font-medium md:font-normal md:mb-0.5">PLAYERS</h4>
          <p className="text-[#FCF8DB] text-sm md:text-base font-semibold">
            {players}
          </p>
        </div>
      ...
      <td className={`${getTdClasses(false, true)} md:rounded-tr-2xl md:rounded-br-2xl`}>
         <div className="w-full md:flex md:justify-center md:items-center h-full">
           <button className="bg-black w-full md:w-auto px-4 py-2 rounded-md text-white text-xs font-bold flex items-center justify-center gap-1 mt-2 md:mt-0 text-nowrap">
             JOIN ➜
           </button>
         </div>
      </td>
