...
import { motion, AnimatePresence } from "framer-motion"; // AnimatePresence is key for tab content
...
const CreateWagerBanner = () => {
  ...
  const TableRow = ({
    ...
    <motion.tr
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      layout // For smooth reordering if list changes
    >
      {children}
    </motion.tr>
  );
  ...
  const renderContent = () => {
    let dataList, isLoading, emptyMessage, actionText, ActionIcon;
    const commonColumns = [
      "Game ID",
      "Game Name",
      "Total Amount",
    ...
    switch (activeTab) {
      ...
      case "invitations":
        ...
        emptyMessage = "No pending invitations at the moment.";
        actionText = "JOIN";
        ActionIcon = UserPlus;
        break;
      case "ongoing":
        ...
        actionText = "VIEW"; // Could be dynamic based on game.nextAction
        ActionIcon = Eye;
        break;
      default:
        return null;
        ...
        {isLoading ? (
          ...
          <table className="min-w-full">
            ...
            <tbody className="divide-y divide-gray-800/50">
              {dataList.map(
                (
                  item: any // Use 'any' for simplicity with different data types, refine with generics if needed
                ) => (
                  <TableRow
                    key={item.id}
                    onClick={
          ...
          <EmptyState message={emptyMessage} icon={InfoIcon} />
        )}
      </motion.div>
    );
  };
  ...
  const TabButton = ({
    ...
    <button
      ...
      style={{ WebkitTapHighlightColor: "transparent" }} // For mobile tap effect
    >
      <Icon
        size={16}
      ...
      {isActive && (
        <motion.div
          ...
          layoutId="activeTabIndicator" // Animate this
          initial={false}
